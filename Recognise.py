...
classifier = load_model('ASLModel.h5') # load trained cnn model
...
def predictor():
       ...
       test_image = image.load_img('1.png', target_size=(64, 64)) # load image to infer
       test_image = image.img_to_array(test_image) # convert image to numpy array
       test_image = np.expand_dims(test_image, axis = 0) # remove extra dimensions
       result = classifier.predict(test_image) # pass image to model for prediction
